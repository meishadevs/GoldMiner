...
var TongTianHeLayer = cc.Layer.extend(
{
    ctor:function ()
    {
        ...
        {
            //播放背景音乐
            cc.audioEngine.playMusic(res.sound003_mp3, true);
        }
        else
        {
            //停止播放背景音乐
       ...
       this.addHaiGui();
        ...
        addPlayer(this, "ssshou01.png", cc.p(430, 410));
    ...
    addHaiGui:function()
    {
        ...
        frameCache.addSpriteFrames(res.haigui_plist, res.haigui_png);
        ...
        for (var i = 1; i <= 6; i++)
        {
            frame = frameCache.getSpriteFrame("hg0" + i + ".png")
        ...
        //将动画包装成一个动作
        var an = new cc.Animate(animation);
        var seq = cc.sequence(an, an.reverse());

        var haigui = new cc.Sprite();
        haigui.x = width / 2;
        haigui.y = height / 2;
        moveNode(haigui, cc.p(0, 80));
        this.addChild(haigui);
        haigui.runAction(cc.repeatForever(seq));
    },
    ...
    createPlayerAnimation:function()
    {
        ...
        {
            //从精灵帧缓存池中读取精灵帧对象
            frame = spriteFrameCache.getSpriteFrame("ssshou0" + i + ".png")
            animation.addSpriteFrame(frame);
        }
        ...
        repShengzi = cc.repeatForever(sp);
    ...
    {
        //创建粒子
        particle = new cc.ParticleSystem(res.paopao_plist);
        particle.x = width / 2;
        particle.y = 0;
    ...
    {
        //通天河场景的过关目标分
        vecScore = [1010, 1240, 1430, 1850, 1880, 1890, 1920, 2000, 2170, 2290, 2530];
        ...
        spriteFrameCache.addSpriteFrames(res.shaseng_plist, res.shaseng_png);
    },
